...
3. **Finalize** --- every rank writes its own data into the same output file;
   ...
   LIKWID and line-profiler records use the token writer even when parallel
   HDF5 is installed. LIKWID reads counters in a subprocess, and Open MPI does
   not safely support a collective MPI-IO phase after that fork. The direct
   writer deliberately uses point-to-point traffic only.
