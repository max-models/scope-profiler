...
run_2.h5  (2 rank(s))
  ...
  -----------------------------------------------
  main              1      1    0.16637   0.16637
  matmul            1      3   0.125949  0.041983
  memory_bound      1      1   0.035121  0.035121
  -----------------------------------------------
  TOTAL                    5    0.32744
```
...
`dvmuzqd /_static/figures/cli/gantt_plot.png :alt: Gantt chart of the mock solver at two MPI ranks :width: 100%`
...
`flame_plot.png` answers *where the time went* instead. The call stack
is reconstructed from timestamp containment — a region whose interval
lies inside another’s is drawn one level above it:

`dvmuzqd /_static/figures/cli/flame_plot.png :alt: Flame graph of the mock solver, rank 0 :width: 100%`
...
time is shown; `--metrics` selects which statistics to include:

``` bash
scope-profiler plot durations run_2.h5 -o figures --metrics avg total
```

`dvmuzqd /_static/figures/cli/durations_plot_avg.png :alt: Average duration per call, by region :width: 100%`

`dvmuzqd /_static/figures/cli/durations_plot_total.png :alt: Total duration, by region :width: 100%`
...
three times as much total time. With several input files, bars are
grouped per region so runs can be compared side by side.
...
`dvmuzqd /_static/figures/cli/duration_timeseries_plot.png :alt: Region duration over wall-clock time :width: 100%`
...
panel per file (Gantt, flame) or groups the files together (durations),
and a speedup plot is added:

``` bash
scope-profiler plot default run_1.h5 run_2.h5 run_4.h5 -o figures_scaling
```

`dvmuzqd /_static/figures/cli/speedup_plot.png :alt: Per-region speedup at 1, 2 and 4 MPI ranks :width: 100%`
...
`dvmuzqd /_static/figures/cli/gantt_plot_filtered.png :alt: Gantt chart restricted to the solve and assemble regions on rank 0 :width: 100%`
