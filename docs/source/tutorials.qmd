...
> **Install for all tutorials:**
> `pip install "scope-profiler[notebook,pproc,line-profiler]"`. Tutorials 1
> and 4 use only the base install; tutorial 2 also needs pandas (`pproc`),
> tutorials 3 and 6 need `pproc`, and tutorial 7 needs all three extras.
...
Seven notebooks that build on each other. Each one is self-contained --- it
generates its own profiling data in a temporary directory, so they can be run
in any order and in any environment.

1. [Getting started](tutorials/01_getting_started.md) — configure the
   profiler, mark regions, finalize, and inspect the first output file.
2. [Post-processing profiling data](tutorials/02_postprocessing.md) — use
   `ProfilingResults`, summaries, DataFrames, filtering, metadata, and JSON.
3. [Visualizing results](tutorials/03_visualization.md) — create Gantt,
   flame, duration, and speedup charts.
4. [Profiling modes and configuration](tutorials/04_profiling_modes.md) —
   recursive, line-by-line, CLI, MPI, and LIKWID profiling.
5. [Custom analysis with the Python API](tutorials/05_custom_analysis.md) —
   inspect individual calls, timelines, call stacks, and self-time.
6. [Building your own plots](tutorials/06_custom_plots.md) — draw custom
   visualizations from events and call-stack data.
7. [Notebook magics](tutorials/07_notebook_magics.md) — profile a cell
   with `%%scope`, `%%scope_line` and `%%scope_recursive`, then compare,
   analyse and export runs with `%scope_compare`, `%scope_df`,
   `%scope_load` and `%scope_export`.

The single command above installs everything. For a smaller environment, use
only the extras named beside the tutorials you plan to run.
