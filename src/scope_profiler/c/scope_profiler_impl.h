 ...
 *
 * This file is also included by scope_profiler.hpp when SP_HEADER_ONLY is
 * defined. Public definitions become C++17 inline functions/variables in
 * that mode, so one profiler state is shared across translation units.
 */
#ifndef SCOPE_PROFILER_IMPL_H
#define SCOPE_PROFILER_IMPL_H

#if defined(SP_HEADER_ONLY) && defined(__cplusplus)
#  if __cplusplus < 201703L
#    error "SP_HEADER_ONLY requires C++17 or newer"
#  endif
#  define SP_IMPL_API inline
#  define SP_IMPL_GLOBAL inline
#else
#  define SP_IMPL_API
#  define SP_IMPL_GLOBAL static
#endif
...
SP_IMPL_GLOBAL clockid_t clock_id = (clockid_t)-1;
SP_IMPL_GLOBAL int clock_resolved = 0;

/* The profiler behind sp_init()/sp_region()/sp_begin()/sp_end()/sp_finalize(). */
SP_IMPL_GLOBAL sp_profiler *g_default = NULL;
...
SP_IMPL_API int64_t sp_now_ns(void)
{
    struct timespec ts;
...
SP_IMPL_API const char *sp_error_string(sp_status status)
{
    switch (status) {
...
SP_IMPL_API int sp_profiler_is_active(const sp_profiler *profiler)
{
    return profiler != NULL && profiler->active;
}

SP_IMPL_API sp_status sp_profiler_last_error(const sp_profiler *profiler)
{
    return profiler != NULL ? profiler->last_error : SP_OK;
}

SP_IMPL_API int sp_profiler_num_regions(const sp_profiler *profiler)
{
    return profiler != NULL ? profiler->n_regions : 0;
}

SP_IMPL_API const char *sp_profiler_region_name(const sp_profiler *profiler, int region)
{
...
SP_IMPL_API sp_profiler *sp_create(const char *prefix, int rank)
{
...
SP_IMPL_API void sp_destroy(sp_profiler *profiler)
{
    int i;
...
SP_IMPL_API int sp_profiler_region(sp_profiler *profiler, const char *name)
{
    return sp_profiler_region_impl(profiler, name, NULL, -1);
}

SP_IMPL_API int sp_profiler_region_at(
    sp_profiler *profiler,
    const char *name,
    const char *source_file,
    int source_line)
...
SP_IMPL_API void sp_profiler_begin(sp_profiler *profiler, int region)
{
    sp_region_t *r;
...
SP_IMPL_API void sp_profiler_end(sp_profiler *profiler, int region)
{
    sp_region_t *r;
...
SP_IMPL_API int sp_profiler_end_last(sp_profiler *profiler)
{
    int region;
...
SP_IMPL_API sp_scope sp_profiler_scope_begin(sp_profiler *profiler, int region)
{
    sp_scope scope;
...
SP_IMPL_API int sp_scope_end(sp_scope *scope)
{
    sp_profiler *profiler;
    sp_region_t *r;
...
SP_IMPL_API int sp_profiler_reset(sp_profiler *profiler)
{
    int i;
...
SP_IMPL_API int sp_profiler_get_region_stats(
...
SP_IMPL_API const char *sp_profiler_output_path(const sp_profiler *profiler)
{
    if (profiler == NULL) {
        return NULL;
    }
...
SP_IMPL_API int sp_init(const char *prefix, int rank)
{
...
SP_IMPL_API int sp_region(const char *name)
{
    return sp_profiler_region(g_default, name);
}

SP_IMPL_API int sp_region_at(const char *name, const char *source_file, int source_line)
{
    return sp_profiler_region_at(g_default, name, source_file, source_line);
}

SP_IMPL_API void sp_begin(int region)
{
    sp_profiler_begin(g_default, region);
}

SP_IMPL_API void sp_end(int region)
{
    sp_profiler_end(g_default, region);
}

SP_IMPL_API int64_t sp_num_calls(int region)
{
    sp_region_stats stats;
...
SP_IMPL_API sp_profiler *sp_default_profiler(void)
{
    return g_default;
}

#undef SP_IMPL_API
#undef SP_IMPL_GLOBAL

#endif /* SCOPE_PROFILER_IMPL_H */
